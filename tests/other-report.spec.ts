/**
 * JobSafe web — Other Report creation form.
 *
 * Entry point: Incident Reports list → "Add New" → "Other" card.
 * The URL is /app/settings/incident-reports/other-report.
 *
 * Required fields (5): Incident Time, Report Type, Title, Severity Level, Employee Signature.
 *
 * Key difference from Near Miss: Report Type is NOT pre-filled. The user must pick one of
 * 8 subtypes (Slip/Trip/Fall, Third Party Injury, Medical, Personal Accident, Vehicle
 * Accident, Physical/Verbal Abuse, Negligence, Other) from an alertdialog. This adds a
 * fifth required field.
 *
 * When subtype "Other" is selected and saved, the reports list shows the type as "Other Incident".
 */
import { test, expect } from '@playwright/test';
import type { Page } from '@playwright/test';
import { LoginPage } from './pages/loginPage';
import { IncidentReportsPage } from './pages/incidentReportsPage';
import { OtherReportFormPage } from './pages/otherReportFormPage';
import { env } from '../utils/env';

const hasCreds = Boolean(env.email && env.password);

/** Navigate login → incident-reports list → "Add New" → "Other" card. */
async function reachOtherReportForm(page: Page) {
  ...
  await ir.otherCard.click();

  const form = new OtherReportFormPage(page);
  await form.expectLoaded();
  await page.waitForLoadState('networkidle');
  return form;
}

test.describe('JobSafe web — Other Report creation', () => {
  test.skip(!hasCreds, 'Missing USER_EMAIL or USER_PASSWORD');

  // ─── Page load ─────────────────────────────────────────────────────

  test('reaches the Other Report form via the real user journey', async ({ page }) => {
    const form = await reachOtherReportForm(page);
    await form.expectLoaded();
  });
  ...
  // test('Report Type field is visible and empty (not pre-filled)', async ({ page }) => {
  //   const form = await reachOtherReportForm(page);
  //   await expect(form.reportTypeField).toBeVisible();
  //   await expect(form.reportTypeField.getByRole('button')).not.toBeVisible();
  // });
  ...
  test('selecting a Report Type removes its required error', async ({ page }) => {
    ...
    await form.saveDraftButton.click();
    await form.expectRequiredFieldCount(5);
    await form.selectReportType('Other');
  ...
  test('fills all required fields and saves an Other Report as draft', async ({ page }) => {
    const form = await reachOtherReportForm(page);
    const title = `Automated Other Draft ${Date.now()}`;
    await form.fillRequiredFields({ title });
    await form.saveDraft();
    await form.expectSavedToReportList(title);
  });

  test('saved Other Report draft shows correct type and "Draft" status in the list', async ({ page }) => {
    const form = await reachOtherReportForm(page);
    const title = `Other Status Check ${Date.now()}`;
    await form.fillRequiredFields({ title, reportType: 'Other' });
    await form.saveDraft();
    await expect(page).toHaveURL(/incident-reports$/, { timeout: 20_000 });
    await form.expectSavedToReportListWithValidStatusAndType(title, 'Draft', 'Other Incident', page);
  });

  test('fills required fields plus optional Description and Severity "High", then saves', async ({ page }) => {
    const form = await reachOtherReportForm(page);
    const title = `Other With Desc ${Date.now()}`;
    await form.fillIncidentTime('10:00 AM');
    await form.selectReportType('Medical');
    await form.fillTitle(title);
    await form.fillDescription('Detail about the incident.');
  ...
  test('fills all required fields and save & send with location service enabled saves successfully', async ({ page, context }) => {
    ...
    await form.expectSavedToReportList(title);
    await form.expectSavedToReportListWithValidStatusAndType(title, 'Pending', 'Other Incident', page);
  });
});
